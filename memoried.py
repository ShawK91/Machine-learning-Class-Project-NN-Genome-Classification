...
if __name__ == "__main__":
    ...
    for inits in range(parameters.num_inits):
        gridworld = mod.Gridworld(parameters)  # Create gridworld
        tracker = mod.statistics(parameters, inits)  # Initiate tracker
        for gen in range (parameters.total_generations): #Main Loop
            ...
            if parameters.use_neat and not parameters.use_py_neat :
                print 'Agent: ', inits, 'Gen:', gen, ' D' if parameters.D_reward else ' G',  ' Best g_reward', int(best_global * 100), ' Avg:', int(100 * tracker.avg_fitness), '  BEST HOF SCORE: ', best_hof_score,  '  Fuel:', 'ON' if parameters.is_fuel else 'Off' , '  Time_offset type:', 'Hard' if parameters.is_hard_time_offset else 'Soft', 'Time_offset: ', parameters.time_offset #, 'Delta MPC:', int(tracker.avg_mpc), '+-', int(tracker.mpc_std), 'Elapsed Time: ', elapsed #' Delta generations Survival: '      #for i in range(num_agents): print all_pop[i].delta_age / params.PopulationSize,
