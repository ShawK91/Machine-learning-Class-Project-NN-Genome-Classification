...
class keras_Population(): #Keras population
    ...
    def init_net(self, input_size, hidden_nodes, output):
        ...
        from keras.optimizers import SGD
        model = Sequential()
        model.add(Dense(hidden_nodes, input_dim=input_size, init='he_uniform', W_regularizer=l2(0.01)))
        # model.add(Activation('sigmoid'))
        model.add(
            SReLU(t_left_init='zero', a_left_init='glorot_uniform', t_right_init='glorot_uniform', a_right_init='one'))
        model.add(Dense(output, init='he_uniform'))
...
class Evo_net():
    def __init__(self, parameters):
        ...
        if parameters.use_deap:
            ...
            self.pop = keras_Population(parameters.evo_input_size, parameters.keras_evonet_hnodes, 5, parameters.population_size)
...
class statistics(): #Tracker
    def __init__(self, parameters, init):
        ...
        self.train_x = [[] for i in range(parameters.num_agents)]
        self.train_y = [[] for i in range(parameters.num_agents)]
    ...
    def save_data(self, gridworld, generation):
        if not os.path.exists('Training Data'):
            os.makedirs('Training Data')

        for agent_id, agent in enumerate(gridworld.agent_list):
            for i in range(self.parameters.population_size):
                self.train_x[agent_id].append(agent.evo_net.pop[i])
                self.train_y[agent_id].append(agent.evo_net.fitness_evals[i])
    ...
    def write_data_csv(self):
        for agent_id in range(len(self.train_x)):
            if not os.path.exists('Training Data/' + str(agent_id)):
                os.makedirs('Training Data/' + str(agent_id))
            ig_x = np.array(self.train_x[agent_id]).astype('f8')
            ig_y = np.array(self.train_y[agent_id]).astype('f8')
            print ig_y.shape, ig_x.shape
            np.savetxt('Training Data/'+str(agent_id) + '/train_x_' + str(self.init) +'.csv', ig_x)
            np.savetxt('Training Data/' + str(agent_id) + '/train_y'+ str(self.init) +'.csv', ig_y)
