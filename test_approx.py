...
def init_keras_model(num_input, num_hnodes, num_output):
...
def load_keras_models(model, num_agents, foldername = 'Models/'):
    import copy
    all_models = []
...
def load_policy_approximator():
    ig_model = init_keras_model(parameters.deap_param.total_num_weights, 100, 1)
    policy_approx = load_keras_models(ig_model, parameters.num_agents)
    return policy_approx

policy_approx = load_policy_approximator()

tracker = mod.statistics(parameters,None) #Initiate tracker
...
def policy_fitness(policy, agent_index):
    policy = np.reshape(policy, (1, len(policy)))
    fit = policy_approx[agent_index].predict(policy)
    return fit[0]
...
def evolve(gridworld, parameters, generation, best_hof_score):
    ...
    for genome_ind in range(parameters.population_size): #For evaluation
        ...
        for id, agent in enumerate(gridworld.agent_list):
            #ig_reward = rewards[id] #normal fitness
            #use policy approximator fitness assignment
            ig_reward = policy_fitness(agent.evo_net.pop[genome_ind], id)
            ig_reward = ig_reward[0]


            agent.evo_net.fitness_evals[genome_ind].append(ig_reward) #Assign those rewards

    #tracker.save_data(gridworld, generation) #Save data
